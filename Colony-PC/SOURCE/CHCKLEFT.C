 ...
 * checkleft.c
 *---------------------------------------------------------------------- 
 * checkleft(xs,ys,xf,yf,left,right,rx,ry,cellx,celly,len)
...
ClipRect(&Clip);
/*move the rotates one cell left*/
rx=rx-tsin;
ry=ry-tcos;
...
while(i<len && left<=right)
	...
	if(Wall[xs][ys] & Side)
		...
		Clip.left=(left=max(drX[xf][yf],left));
		ClipRect(&Clip);
		}
	...
	if(i<len&&left<=right)
		...
		while((!(Wall[xs][ys]&Side))&&i<len)
			...
			if(drX[xf+Frntx][yf+Frnty]>left)
				...
				if(Wall[cellx][celly] & ~0x03)
					...
					xf2+Frntx,yf2+Frnty,
					left,Clip.right,rx,ry);
				if(robotarray[cellx][celly])
							SetRobot(left,Clip.right,
							robotarray[cellx][celly]);
				if(foodarray[cellx][celly])
							SetRobot(left,Clip.right,
							foodarray[cellx][celly]);
				}
			else
				{
				j=0;
		 ...
		 */
		if(Wall[xf-Sidex][yf-Sidey]&Front)
			{
			MoveTo(drX[xf2][yf2],drY[xf2][yf2]);
			...
			saveClip=Clip.right;
			if((min(drX[xf2][yf2],right))>=left)
			checkleft(xsstart,
				ysstart,
				xfstart,
				yfstart,
				left,
				min(right,drX[xf2][yf2]),
